...
export default function Transfer(props: TransferContainerProps): ReactElement {
    ...
    const data = useMemo<TransferItem[]>(
        () => {
            const d = props.options.items?.map<TransferItem>(option => ({
                key: props.keyAttribute.get(option).value?.toString(), title: props.titleAttribute.get(option).value
            }));
            return d ? d : [];
        }
        , [props.options]);
    ...
    return (
        <TransferContainer targetKeys={props.targetKeys.value!} onChange={onChange} isShowSearch={props.isShowSearch} leftTitle={props.leftTitle?.value!} rightTitle={props.rightTitle?.value!} data={data} isDisabled={props.isDisable} />
    )
}
