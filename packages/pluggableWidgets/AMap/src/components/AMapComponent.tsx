...
export const AMapComponent = (props: AMapComponentProps) => {
    ...
    return <div style={{ width: '100%', height: 600 }}>
        <Map
            ...
            zoom={props.zoom}
            onZoomChange={() => {
                props.change && props.change(mapRef.current?.map.getCenter().lat, mapRef.current?.map.getCenter().lng);
            ...
            <ControlBarControl offset={[16, 180]} position="RB" />
            {
                props.marks.map((mark, idx) =>
                    <Marker onDblClick={(e: any) => {
                        props.onDblClick && props.onDblClick(e, idx);
